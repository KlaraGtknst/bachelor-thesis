...
one can determine whether two points are in the same cluster for a given information saved by OPTICS \cite{OPTICS2013, OPTICS1999}.
...
Similar to the approach from \cite{OPTICS1999}, OPTICS was used to cluster the images of the first page of documents in this work.
The images were preprossed to 32x32 greyscale pixel as visualized in \autoref{fig:preprocessed_docs}.
The reachability distance ordered by OPTICS is displayed in \autoref{fig:reachability_plot}.
...
\begin{figure}[htp] % htp = hier (h), top (t), oder auf einer eigenen Seite (p).
    ...
    The plot was created using the OPTICS algorithm from the Python library scikit-learn.
...
% what docs are in the cluster, what are noise?
\begin{figure}%
    \centering
    \subfloat[\centering Example image from cluster 0. The image seems to be a normal, non-certficate document.]{{\includegraphics[width=5cm]{images/example_image_cluster_0.pdf} }}%
    \qquad
    \subfloat[\centering Example image which was identified as noise. The image seems to be a certficate.]{{\includegraphics[width=5cm]{images/example_image_cluster_-1.pdf} }}%
    \caption{When using OPTICS with the configurations stated above, only one cluster and noise was identfied.
